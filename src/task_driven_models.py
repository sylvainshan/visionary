...
# === Parameters ====
SAVE_FIG = True
FIG_PATH = f'Figures/task_driven_models'
DATA_PATH =f'data/task_driven_models'
RANDOM_SEED = 42
BATCH_SIZE = 100
N_PCA_COMPONENTS = np.arange(1, 10) * 100
...
for n_pca in N_PCA_COMPONENTS:
    ...
    if not os.path.exists(DATA_PATH + f"/{n_pca}pcs/pretrained_results.pkl"):
        ...
        with open(DATA_PATH + f"/{n_pca}pcs/pretrained_results.pkl", "rb") as f:
            pretrained_results = pickle.load(f)

    # Evaluate the ResNet50 random model
    if not os.path.exists(DATA_PATH + f"/{n_pca}pcs/random_results.pkl"):
        ...
        with open(DATA_PATH + f"/{n_pca}pcs/random_results.pkl", "rb") as f:
